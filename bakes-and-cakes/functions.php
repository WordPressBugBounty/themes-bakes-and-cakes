...
function bakes_and_cakes_scripts() {
	...
	if( get_theme_mod( 'ed_localgoogle_fonts',false ) && ! is_customize_preview() && ! is_admin() ){
        if ( get_theme_mod( 'ed_preload_local_fonts',false ) ) {
			bakes_and_cakes_load_preload_local_fonts( bakes_and_cakes_get_webfont_url( bakes_and_cakes_fonts_url() ) );
        }
        wp_enqueue_style( 'bakes-and-cakes-google-fonts', bakes_and_cakes_get_webfont_url( bakes_and_cakes_fonts_url() ) );
    }else{
    ...
    wp_enqueue_script( 'bakes-and-cakes-modal-accessibility', get_template_directory_uri() . '/js' . $build . '/modal-accessibility' . $suffix . '.js', array( 'jquery' ), BAKES_AND_CAKES_THEME_VERSION, true );
...
function bakes_and_cakes_customizer_js() {
	$array = array( 
        ...
        'flushit'    => __( 'Successfully Flushed!','bakes-and-cakes' ),
	...
	wp_localize_script( 'bakes-and-cakes-customizer-js', 'bakes_and_cakes_cdata', $array );
}
endif;
